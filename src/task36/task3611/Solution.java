...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        if (deep == 0) {
            ...
            for (int p = 0; p < humanRelationships.length; p++) {
                if ((p < index) && (index < humanRelationships.length) && humanRelationships[index][p]) {
                    result.add(p);
                } else if ((p > index) && humanRelationships[p][index]) {
                    result.add(p);
                }
            }
